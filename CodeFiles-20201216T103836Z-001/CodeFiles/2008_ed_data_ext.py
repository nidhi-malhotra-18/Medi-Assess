...
for i in range(0,len(data)):
    # age_data = int(data[i][7:10]) # 
    # sex_data = int(data[i][24]) #
    # temp_data = float(data[i][41:45]) 
    # hr_data = float(data[i][45:48]) 
    # rr_data = float(data[i][48:50]) 
    # sbp_data = float(data[i][50:53])
    # dbp_data = float(data[i][53:56])
    # po_data = float(data[i][56:59]) 
    # oxygen_data = int(data[i][59:61]) 
    # pain_data = int(data[i][65:67]) 
    # # result_data = int(data[i][66:68])
    # death_in_ed = int(data[i][358]) #
    # icu_data = int(data[i][365:367]) # 
    # hospital_discharge_status = int(data[i][382:384]) #
    # arrival_ambulance = int(data[i][29:31]) #
    # length_of_visit = int(data[i][373:377]) #
    # obs_unit_discharge = int(data[i][363])  
    # esi_data = int(data[i][61:62]) 
    age_data = int(data[i][7:10]) # 
    sex_data = int(data[i][24]) #
    temp_data = float(data[i][41:45]) 
    hr_data = float(data[i][45:48]) 
    ...
    po_data = float(data[i][56:59]) 
    oxygen_data = int(data[i][59:61]) 
    pain_data = int(data[i][63:65]) 
    # result_data = int(data[i][66:68])
    death_in_ed = int(data[i][352]) #
    icu_data = int(data[i][359:361]) # 
    hospital_discharge_status = int(data[i][370:372]) #
    arrival_ambulance = int(data[i][29:31]) #
    length_of_visit = int(data[i][361:365]) #
    obs_unit_discharge = int(data[i][363])
...
df_final = pd.DataFrame(data_final, columns = ['Age','Sex','Temperature','Heart Rate','Resp Rate' ,
    'Systolic BP','Diastolic BP','Pulse Oximetry','Oxygen','Pain','Arrival','Result', 'Length Of Stay','Reason1','Reason2','Reason3'])

df_final.to_csv('data_2008_reasons.csv')